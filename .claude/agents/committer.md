...
**Numbers and currency use Spanish typography, even though the prose is English.** Comma as the
decimal separator, point as the thousands separator, the currency symbol after the figure with a
space, and a space before `%`: `1.090,00 €`, `14.415 €`, `9 %`. Never `€1,090.00`, never `9%`.

Values quoted straight from the code keep the form they have there: if a test asserts
`'0.007207'`, write it that way, not `0,007207`.

Everything else follows English prose: em dash for asides (`—`) and straight quotes.
